function loadEventsData(this)
    ...
    for ne = rawEvents(locNormal)
        ...
        eventDetails = regexp(ne.details, mag.event.Event.CommonFormat + ...
            ", NORMPRI_RATE=(?<primary>HZ_\d+|\w+), NORMSEC_RATE=(?<secondary>HZ_\d+|\w+), NORM_PKTSECS=(?<packets>SECS_\d+|\w+), BRSTPRI_RATE=(?:HZ_\d+|\w+), BRSTSEC_RATE=(?:HZ_\d+|\w+), BRST_PKTSECS=(?:SECS_\d+|\w+)", "once", "names");
        eventDetails = structfun(@(x) replace(x, ["SEC_", "HZ_", "UNCHANGED"], ["", "", "NaN"]), eventDetails, UniformOutput = false);
    ...
    for be = rawEvents(locBurst)
        ...
        eventDetails = regexp(be.details, mag.event.Event.CommonFormat + ...
            ", NORMPRI_RATE=(?:HZ_\d+|\w+), NORMSEC_RATE=(?:HZ_\d+|\w+), NORM_PKTSECS=(?:SECS_\d+|\w+), BRSTPRI_RATE=(?<primary>HZ_\d+|\w+), BRSTSEC_RATE=(?<secondary>HZ_\d+|\w+), BRST_PKTSECS=(?<packets>SECS_\d+|\w+), BRST_DURATION=(?<duration>\d+)", "once", "names");
        eventDetails = structfun(@(x) replace(x, ["SEC_", "HZ_", "UNCHANGED"], ["", "", "NaN"]), eventDetails, UniformOutput = false);
