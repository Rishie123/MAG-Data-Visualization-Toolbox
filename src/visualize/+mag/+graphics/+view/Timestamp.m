    ...
    methods
        ...
        function visualize(this)
            ...
            this.Figures(1) = mag.graphics.visualize( ...
                primaryComparison, ...
                mag.graphics.style.Default(Title = compose("%s vs. %s", primarySensor, secondarySensor), YLabel = "\Deltat [ms]", YScale = "log", Layout = [2, 1], Charts = mag.graphics.chart.Plot(XVariable = "p", YVariables = this.getScienceTimingOperation("p", "s"), Filter = primaryScience.Quality.isScience())), ...
                secondaryComparison, ...
                mag.graphics.style.Default(Title = compose("%s vs. %s", secondarySensor, primarySensor), YLabel = "\Deltat [ms]", YScale = "log", Layout = [2, 1], Charts = mag.graphics.chart.Plot(XVariable = "s", YVariables = this.getScienceTimingOperation("s", "p"), Filter = secondaryScience.Quality.isScience())), ...
            ...
            if ~isempty(this.Results.IALiRT)
                ...
                if numel(primaryIALiRT.Time) == numel(secondaryIALiRT.Time)
                    ...
                    this.Figures(2) = mag.graphics.visualize( ...
                        timestampComparison, ...
                        [mag.graphics.style.Default(Title = compose("I-ALiRT %s vs. %s", primarySensor, secondarySensor), YLabel = "\Deltat [ms]", Layout = [2, 1], Charts = mag.graphics.chart.Plot(XVariable = "pi", YVariables = this.getIALIRTTimingOperation("pi", "si"))), ...
                        mag.graphics.style.Default(Title = compose("%s Science vs. I-ALiRT", primarySensor), YLabel = "\Deltat [ms]", Layout = [2, 1], Charts = mag.graphics.chart.Plot(XVariable = "ps", YVariables = this.getIALIRTTimingOperation("ps", "pi"))), ...
                        mag.graphics.style.Default(Title = compose("%s Science vs. I-ALiRT", secondarySensor), YLabel = "\Deltat [ms]", Layout = [2, 1], Charts = mag.graphics.chart.Plot(XVariable = "ss", YVariables = this.getIALIRTTimingOperation("ss", "si")))], ...
                        primaryScience.Events, mag.graphics.style.Default(Title = compose("%s Modes", primarySensor), YLabel = "mode [-]", YLimits = "manual", Charts = mag.graphics.chart.custom.Event(EventOfInterest = "DataFrequency", EndTime = primaryScience.Time(end))), ...
                        secondaryScience.Events, mag.graphics.style.Default(Title = compose("%s Modes", secondarySensor), YLabel = "mode [-]", YLimits = "manual", Charts = mag.graphics.chart.custom.Event(EventOfInterest = "DataFrequency", EndTime = secondaryScience.Time(end))), ...
